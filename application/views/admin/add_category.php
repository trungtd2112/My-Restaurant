...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
    <div class="row">
        <ol class="breadcrumb">
            ...
            <li class="active">Quản lý danh mục Food</li>
        </ol>
    </div>
    <!--/.row-->
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Quản lý danh mục Food</h1>
        </div>
    </div>
    <!--/.row-->
    <div class="row">
                ...
                <div class="panel-body">
                    <div class="col-md-8">
                        <span class="btn btn-success" id="add">
                            <i class="glyphicon glyphicon-plus "></i> Thêm danh mục Food
                        </span>
                        <div id="warning" class="hidden alert alert-danger">Danh mục không hợp lệ !</div>
                        <form class="hidden">
                            <div class="form-group ">
                                <label>Tên danh mục:</label>
                                <input required type="text" name="dc_name" id="dc_name" class="form-control" placeholder="Tên danh mục...">
                            </div>
                    ...
                    <table data-toolbar="#toolbar" data-toggle="table" id="categories">
                        ...
                        <tbody>
                            <?php foreach ($category_data as $key => $value) { ?>
                                <tr>
                                    <td><?php echo $value['dc_id'] ?></td>
                                    <td><?php echo $value['dc_name'] ?></td>
                                    <td><?php echo date('d/m/y', $value['dc_date']); ?></td>
                                    <td class="form-group">
                                        <a data-href="<?php echo $value['dc_id'] ?>" class="btn btn-primary edit_button"><i class="glyphicon glyphicon-pencil"></i></a>
                                        <a data-href="<?php echo $value['dc_id'] ?>" class="btn btn-danger delete_button"><i class="glyphicon glyphicon-remove"></i></a>
                                    </td>
                                    <!-- form edit -->
                                    <td>
                                        <form role="form" method="post" class="jquery_edit_form hidden">
                                            <div class="form-group">
                                                <label>Tên danh mục:</label>
                                                <input type="text" name="dc_name" required value="<?php echo $value['dc_name'] ?>" class="form-control" placeholder="Tên danh mục...">
                                            </div>
...
<script>
    $(function() {
        ...
        $('body').on('click', '#addCat', function() {
            ...
            $.ajax({
                    type: "post",
                    url: "<?php echo base_url() ?>" + "admin/Category/addJquery",
                    data: {
                        'dc_name': $('#dc_name').val()
                    },
                    dataType: "json"
                ...
                .always(function(res) {
                    ...
                    if (res != 0) {
                        ...
                        content += '123';
                        content += '</td>';
                        content += '<form role="form" method="post" class="jquery_edit_form hidden">';
                        content += '<div class="form-group">';
                        content += '<label>Tên danh mục:</label>';
                        content += '<input type="text" name="dc_name" required value="" class="form-control" placeholder="Tên danh mục...">';
                        content += '</div>';
                        content += '<button type="submit" name="sbm" class="btn btn-primary jquery_edit_button">Cập nhật</button>';
                        content += '<button type="reset" class="btn btn-default">Làm mới</button>';
                        content += '</form>';
                        content += '</td>';
                        content += '</tr>';
        ...
        $('html').on('click', '.delete_button', function(event) {
            ...
            $.ajax({
                    type: "post",
                    url: '<?php echo base_url() ?>' + 'admin/Category/deleteDishCategory/' + id,
                    dataType: "json"
                })
                .always(function() {
        ...
        //edit
        $('html').on('click', '.edit_button', function() {
            //hiện form sửa
            if (j % 2 == 0) {
        ...
        $('body').on('click', '.jquery_edit_button', function() {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url() ?>" + "admin/Category/editDishCategory/" + id,
                data: {
                    'dc_name': new_cat
                },
                dataType: "json"
